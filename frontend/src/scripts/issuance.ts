 ...
 */
export async function issueProofForFile(filePath: string, options: Partial<IssuanceOptions> = {}): Promise<IssuanceResult> {
  ...
  try {
    switch (command) {
      ...
      case 'payload':
        ...
        const payloadResult = await issueProofForPayload(payload, 'payload.txt', { output: payloadOutputDir });
        console.log('Proof issued successfully:');
      ...
      default:
        ...
        console.log('  tsx issuance.ts payload <payload> [output-dir]    - Issue proof for payload');
        console.log('  tsx issuance.ts test-reproducibility              - Test reproducibility');
        console.log('  tsx issuance.ts mock-proofs [output-dir]          - Generate 10 mock proofs');
        break;
    }
  } catch (error) {
