...
async function testDatabaseAndStorage(): Promise<ValidationResult[]> {
  ...
  try {
    ...
    try {
      ...
      const { data, error } = await supabase.from("proofs").insert(testProof).select().single();
...
async function testApiEndpoints(): Promise<ValidationResult[]> {
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/api/proof/${
        TEST_DATA.proof.id
      }`,
    );
    results.push({
...
async function runPilotReadinessValidation(): Promise<PilotReadinessReport> {
  ...
  const componentResults: {
    [key: string]: { status: "pass" | "fail" | "warning"; tests: ValidationResult[] };
  } = {};
