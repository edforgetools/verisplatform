...
async function handleVerify(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    capture(error, { route: "/api/check", latency });
...
async function verifyByHash(
  ...
  logger.warn(
    ...
    "Delivery check failed from all sources",
  );
...
async function returnVerificationResult(
  ...
  try {
    await Promise.all([
      ...
      recordApiCall("/api/check", undefined, {
...
export const GET = withRateLimit(handleVerify, "/api/check", {
  capacity: 100, // 100 requests per minute for public checking
  refillRate: 100 / 60, // 100 tokens per minute
  windowMs: 60000, // 1 minute window
});

// Also support POST for file uploads
export const POST = withRateLimit(handleVerify, "/api/check", {
