...
export async function GET(
  ...
  const formattedFingerprint = keyFingerprint
    ? formatKeyFingerprint(keyFingerprint)
    : 'Unknown';
  ...
  doc
    .fontSize(24)
    .font('Helvetica-Bold')
    .text('Veris Proof Certificate', { align: 'center' });
  doc.moveDown(1.5);
  ...
  doc
    .fontSize(14)
    .font('Helvetica-Bold')
    .text('Certificate Metadata:', 50, doc.y);
  doc.moveDown(0.5);

  doc.fontSize(12).font('Helvetica').text(`Issuer: Veris`);
  doc.text(`Public Key Fingerprint: ${formattedFingerprint}`);
  doc.text(
    `Verification Result: ${verificationResult ? 'VERIFIED' : 'FAILED'}`,
  );
  doc.text(
    `Certificate Generated: ${new Date(
      proof.created_at || proof.timestamp,
    ).toUTCString()}`,
  );
  doc.moveDown(1);
  ...
  doc
    ...
    .font('Helvetica')
    .text(
      'This certificate provides cryptographic proof of file integrity.',
      50,
      doc.y,
      {
        width: 500,
