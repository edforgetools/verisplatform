...
async function checkDbHealth(): Promise<HealthCheck> {
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/db-health`, {
      cache: 'no-store'
    });
...
async function checkVersion(): Promise<HealthCheck> {
  const startTime = Date.now();
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/version`, {
      cache: 'no-store'
    });
    const data = await response.json();
    const responseTime = Date.now() - startTime;
          ...
          <div className="px-6 py-8">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex items-center space-x-2">
                <div className={`w-3 h-3 rounded-full ${overallStatus === 'healthy' ? 'bg-green-500' : 'bg-red-500'}`}></div>
                <span className={`text-sm font-medium ${overallStatus === 'healthy' ? 'text-green-600' : 'text-red-600'}`}>
                ...
                <div key={index} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <div className={`w-2 h-2 rounded-full ${check.status === 'healthy' ? 'bg-green-500' : 'bg-red-500'}`}></div>
                      <h3 className="text-lg font-medium text-gray-900">{check.name}</h3>
                    </div>
                    <div className="text-right">
                      <div className={`text-sm font-medium ${check.status === 'healthy' ? 'text-green-600' : 'text-red-600'}`}>
